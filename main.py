...
# Optional DB support (we fall back to in-memory if DATABASE_URL isn't set)
try:
    import psycopg
...
def add_note(session_id: str, note: str) -> None:
    if db_enabled():
        with db_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO memory_notes (session_id, note)
                    VALUES (%s, %s)
                """, (session_id, note))
            conn.commit()
        return
...
def chat(payload: ChatIn, authorization: Optional[str] = Header(default=None)):
    ...
    # Responses API supports messages with roles, and supports an "instructions" param too. :contentReference[oaicite:2]{index=2}
    resp = client.responses.create(
        model=OPENAI_MODEL,
        instructions=instructions,
        input=history,
        store=False,  # disable storage by OpenAI :contentReference[oaicite:3]{index=3}
    )
